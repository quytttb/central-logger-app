...
2. **Cấu trúc thư mục QML (tương đối với `src/central_logger/ui/`):**
   - `main.qml`: Root `ApplicationWindow`; khai báo `LoggerListModel`, `DashboardController`, `SettingsController` và alias xuống các view; routing giữa các view qua `currentView`.
   ...
   - `components/`: Custom UI tái sử dụng — `common/` (Badge, StatusBadge, HoverHighlight, SettingsField), `cards/` (StatCard), `navigation/` (AppSidebar, AppTopBar), `dialogs/` (BaseDialog, AddLoggerDialog, ConfirmDialog, EditConfigDialog), `charts/` (BaseChart), `utils/` (FrameResizeHandles).
   - `UiMotion.qml` (singleton): hằng số animation; `qmldir` cùng cấp khai báo singleton.
